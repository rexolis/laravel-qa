...
<div {{ $attributes->merge(['class' => 'flex flex-col items-center shrink-0 min-w-[60px] mr-8 text-center text-gray-600 dark:text-gray-400']) }}>
    ...
    @if ($variant === 'question')
        <a href="#" title="{{ __('Click to mark as favorite question (Click again to undo)') }}"
            @class([
                'block mt-2',
                'text-amber-400 hover:text-amber-500' => $favorited,
                'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200' => ! $favorited,
            ])>
